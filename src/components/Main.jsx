...
const Main = ({ deletItem, courses, selectCourse, selectedCourse, sum1 }) => {
  return (
    <div className="w-[100%] mx-auto">
      <div className="grid bg-[#f6f7f9 rounded-md max-sm:w-full grid-cols-[1fr,3fr] gap-4 max-md:grid-cols-1 ">
        <div className="bg-white border-gray-300 border-[1px] max-sm:overflow-auto text-white max-sm:h-[30vh] max-sm:w-full rounded-md flex gap-5 flex-col p-4">
          <h4 className="text-xl   text-green-500 uppercase max-sm:sticky  bg-white max-sm:-top-5 "> total {sum1}$</h4>
          {selectedCourse
            ? selectedCourse.map((item, index) => (
                <div key={index} className="flex w-full   justify-between shadow-inner shadow-white rounded-md pl-2 bg-slate-800 items-center ">
                  <h4>{item.title}</h4>
                  <p>{item.price}$</p>
        ...
        <div className="grid grid-cols-2  gap-4 max-md:grid-cols-1">
          {courses.map((item, index) => (
            <div key={index} className="flex cursor-pointer bg-white shadow-md shadow-white flex-col gap-4 p-4 rounded-lg border-slate-300 border-[1px] items-center relative min-h-[30vh] max-sm:h-[20vh]">
